---
id: HAE-concept-duc-hy
title: "Dục Hỷ (Nandīrāga)"
category: "Tứ Thánh Đế (Four Noble Truths)"
tags:
  - ai-duc
  - tham-ai
  - kiet-su
  - luan-hoi
date: 2026-05-31
parent: tuong-tri
---

# Dục Hỷ (Nandīrāga)

## 💡 Định nghĩa & Nội dung Cốt lõi
Sự ham thích, đắm say và bám chấp phát sinh từ tiến trình tưởng tri sai lệch, xem các đối tượng nhận thức là thuộc về mình hoặc chính là bản ngã của mình. Đây chính là gốc rễ của khổ đau và sự ràng buộc trong luân hồi.

## ⚙️ Nguyên lý Kỹ thuật & Thực tiễn
- Phát sinh trực tiếp từ việc tưởng tri các pháp thay vì liễu tri chúng.
- Tạo ra chất xúc tác củng cố ảo tưởng về một tự ngã thụ hưởng.
- Là động lực thúc đẩy hành vi thủ chấp (upādāna) và tạo nghiệp tái sinh.

## 🔗 Mạng lưới Nhân Duyên Quả (Causal Web)
- **Nhân gốc (Causal Core)**: [Tuong Tri](02_atomic_nodes/HAE-concept-tuong-tri.md) — Khái niệm nền tảng sinh ra khái niệm này.
- **Hội tụ Duyên (Supporting Conditions)**: [Ai Duc](02_atomic_nodes/HAE-concept-ai-duc.md), [Nghiệp (Kamma)](02_atomic_nodes/HAE-concept-nghiep.md) — Các khái niệm hỗ trợ trực tiếp.
- **Quả chuyển hóa (Derivative Effects)**: Không có — Các giải pháp và hiệu quả kế thừa.

## 🔗 Liên kết Tri thức (Knowledge Connections)
- **Dẫn chứng & Nguồn gốc (Ngược dòng - Evidence & Context)**:
  - [Ghi chú cấu trúc: Sutta Mn 001](01_structured_docs/sutta-mn-001-processed.md)
  - [Ghi chú thô: Sutta Mn 001](00_raw_docs/sutta-mn-001.md)
- **Đúc kết vĩ mô (Xuôi dòng - Distilled Thoughts)**:
  - [Đúc kết Kinh điển Nikaya](04_distilled/nikaya-distilled.md)
